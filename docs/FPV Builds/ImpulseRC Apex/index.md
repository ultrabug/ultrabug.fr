...
!!! note
    :gb: Those links are **NOT** affiliate links. I just share the links where I actually bought the stuff needed for this build.

    :fr: Les liens ci-dessous ne sont **PAS** des liens d'affiliation. Ce sont tout simplement les liens où j'ai moi-même commandé le matériel nécessaire à ce build.
...
- [ ] Drop the ESC capacitor and replace it with TVS diodes [RUSHFPV RUSH BLADE Power Filter Board](https://www.getfpv.com/rushfpv-rush-blade-power-filter-board.html)

## Build steps

### Frame assembly

:gb: Follow the [official ImpulseRC Apex guide](http://apex-docs.impulserc.com/) to assemble the frame.

:fr: Suivre le [guide officiel de ImpulseRC](http://apex-docs.impulserc.com/) pour assembler la frame.
...
!!! note
    :gb: You will need 4 long M2 screws that are not provided with the kit. Nuts are the ones provided with the kit.

    :fr: Vous aurez besoin de 4 longues vis M2 non incluses dans le kit. Les écrous sont cependant ceux fournis avec le kit.
...
:gb: The **20cm cable for Caddx Vista** is better protected than the standard one.

:fr: Le **câble de 20cm pour Caddx Vista** est mieux protégé que celui d'origine.

![Apex Base Kit 20x20 Caddx Vista cable](images/IMG_20210109_171121.jpg)

### Caddx Vista VTX antenna mounting

:gb: Use the **U.FL TBS Unify Pro/HV SMA** cable with the **provided Apex clamp** and fix it to the rear for the antenna.

:fr: Utiliser le câble **U.FL TBS Unify Pro/HV SMA** avec le **sabot Apex fourni** fixé à l'arrière pour l'antenne.
...
:gb: Do not fear to cut short the XT60 leads: its TPU support will strain releave the cable on the standoff.

:fr: N'ayez pas peur de couper fort les câbles de la prise XT60 : son support TPU va sécuriser le câble contre l'arrachement sur l'entretoise.

![Apex Base Kit XT60 cable cut](images/IMG_20210109_173831.jpg)

!!! warning
    :gb: Be mindful of the fact that **the ESC mounted in reverse position**to leverage the space at the front for the capacitor. You will need to reallocate the motor resources on the CLI (see the betaflight section below).

    :fr: Notez bien que **l'ESC est monté à l'envers** afin d'utiliser l'espace disponible à l'avant pour placer le capacitor. Vous devrez réallouer les ressources moteurs dans la CLI (voir la section betaflight ci-après).
...
:gb: Nothing surprising here, wiring the TBS Tracer Nano RX is straightforward!

:fr: Aucune surprise dans le branchement du TBS Tracer Nano RX !
...
:gb: Use the longest of the provided FC-to-ESC cables (it connects perfectly, mind the wire direction) and connect the Caddx Vista.

:fr: Utilisez le câble FC-vers-ESC fourni le plus long (cela passe parfaitement, attention il y a un sens) et connectez la Caddx Vista.
...
:gb: Once protected, the RX is stuck on top of the Caddx Vista using double sided tape.

:fr: Une fois protégé, le RX tient sur le Caddx Vista grâce à du scotch double face.

![Apex Base Kit RX mount view](images/IMG_20210109_183016.jpg)

:gb: The Tracer Immortal T antennas fit well: one is zip tied in the lower plate by the provided clamp and the other is passing through the remaining hole in the upper VTX clamp.

:fr: Les antennes Tracer Immortal T tiennent bien : une est zippée sur le sabot de la plate du bas fourni et l'autre tient fermement dans le trou du sabot VTX du haut.
...
:gb: Finish the build by placing the top plate and the plastic provided covers on the arms.

:fr: Finissez le build en plaçant la top plate et les caches en plastiques sur les bras fournis.
...
:gb: Since the ESC is mounted in reverse position, the motors need to be reassigned on the CLI. Depending on your wiring, IDs may change.

:fr: Comme l'ESC a été monté à l'envers il est nécessaire de repositionner les moteurs dans la CLI. Selon votre montage, les ID peuvent changer.
...
:gb: This is the minimal CLI diff that I set up without advanced tuning. It flies very well but as I will tune it better I will add a section to this build guide.

:fr: CLI diff de la configuration minimale sans tuning approfondi pour l'instant. Ca vol déjà super bien mais j'ajouterai une section à ce guide lorsque j'ajusterai le tuning.
