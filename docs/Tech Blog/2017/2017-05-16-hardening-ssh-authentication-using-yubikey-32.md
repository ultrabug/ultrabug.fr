...
First, insert your Yubikey and let's change its USB operating mode to OTP+U2F+CCID with MODE\_FLAG\_EJECT flag.
...
_NOTE: if you have an older version of Yubikey (before Sept. 2014), use -m82 instead._
...
_I advise you say Yes to the off-card backup of the encryption key._
...
gpg: Note: backup of card key saved to '/home/ultrabug/.gnupg/sk\_8E407636C9C32C38.gpg'
...
_Make sure to store that backup in a secure and offline location._
...
scp 22A73AED8E766F01.asc user@server:public\_html/static/22A73AED8E766F01.asc
...
You can skip the fetch/import part below if you generated the key on the same machine than you are using it. You should see it listed when executing _gpg -k_.
...
_Note the "Card serial no." showing that the key is actually stored on a smartcard._
...
SSH\_AUTH\_SOCK=/run/user/$(id -u)/gnupg/S.gpg-agent.ssh
...
_This is what ends up in ~/.ssh/authorized\_keys on your servers._
